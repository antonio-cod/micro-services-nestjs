...
describe('AuthModule JWT configuration', () => {
  it('uses the configured secret without signing configuration', () => {
    const configService = {
      get: jest.fn().mockReturnValue(' jwt-secret '),
    } as unknown as ConfigService;

    expect(createJwtOptions(configService)).toEqual({ secret: 'jwt-secret' });
  });
  ...
  it('registers JwtAuthGuard as a global guard', () => {
    ...
    expect(providers).toEqual(
      expect.arrayContaining([{ provide: APP_GUARD, useClass: JwtAuthGuard }]),
    );
  });
});
